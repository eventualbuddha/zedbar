...
static const char *note_usage =
    N_("usage: zbarimg [options] <image>...\n"
       "\n"
       ...
       "    --polygon       output points delimiting code zone with decoded "
       "symbol data\n"
...
static const char *warning_not_found_head =
    N_("\n"
       ...
       "Things to check:\n"
       "  - is the barcode type supported? Currently supported symbologies "
       "are:\n");
      ...
      if (typ == ZBAR_PARTIAL)
        ...
        if (polygon) {
          ...
          if (zbar_symbol_get_loc_size(sym) > 0)
            printf("%+d,%+d", zbar_symbol_get_loc_x(sym, 0),
                   zbar_symbol_get_loc_y(sym, 0));
          for (p = 1; p < zbar_symbol_get_loc_size(sym); p++)
            printf(" %+d,%+d", zbar_symbol_get_loc_x(sym, p),
...
int main(int argc, const char *argv[]) {
    ...
    if (arg[0] != '-' || !arg[1])
      ...
      for (j = 1; arg[j]; j++) {
        ...
        case 'D':
          break;
        default:
          return (usage(1, "ERROR: unknown bundled option: -", arg + j));
        }
      }
  ...
  if (num_images && !quiet && xmllvl <= 0) {
    fprintf(stderr, "scanned %d barcode symbols from %d images", num_symbols,
            num_images);
#ifdef HAVE_SYS_TIMES_H
#ifdef HAVE_UNISTD_H
...
#if ENABLE_EAN == 1
      fprintf(stderr, _("\t. EAN/UPC (EAN-13, EAN-8, EAN-2, EAN-5, UPC-A, "
                        "UPC-E, ISBN-10, ISBN-13)\n"));
#endif
#if ENABLE_DATABAR == 1
