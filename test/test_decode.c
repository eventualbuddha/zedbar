...
static void symbol_handler(zbar_decoder_t *decoder) {
  ...
  if (sym != expect_sym) {
    zprintf(0, "[%d] SEED=%d: warning: expecting %s, got spurious %s\n", iter,
            seed, zbar_get_symbol_name(expect_sym), zbar_get_symbol_name(sym));
    spurious++;
    return;
  }
  ...
  if (!pass) {
    zprintf(0, "[%d] SEED=%d: ERROR: expecting %s (%s), got %s (%s)\n", iter,
            seed, expect_data, zbar_get_symbol_name(expect_sym), data,
...
static uint64_t encode_databar_char(unsigned val, const struct rss_group *grp,
  ...
  zprintf(3, " G_sum=%d T_odd=%d T_even=%d n_odd=%d w_max=%d V_grp=%d\n", G_sum,
          grp->T_odd, grp->T_even, grp->n_odd, grp->w_max, V_grp);
...
int main(int argc, char *argv[]) {
  ...
  if (!wrong && percent(spurious, num_iter) <= 0.01 &&
    ...
    if (spurious || missing)
      printf("decoder PASSED with %d spurious (%02.4f%%) and %d "
             "missing(%02.4f%%).\n",
    ...
    printf("decoder FAILED with %d wrong decoding(%02.4f%%), %d spurious "
           "(%02.4f%%) and %d missing(%02.4f%%).\n",
