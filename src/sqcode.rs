    ...
    pub(crate) fn decode(&mut self, img: &mut ImageData) -> Result<Option<Symbol>, ()> {
        ...
        for y in 3..=border_len - 4 {
            for x in 3..=border_len - 4 {
                ...
                let top_left_color = data[sample_y * img.width as usize + sample_x];

                let sample_x = bottom_right_source_x as usize;
                let sample_y = bottom_right_source_y as usize;
                let bottom_right_color = data[sample_y * img.width as usize + sample_x];
...
fn sq_scan_shape(img: &ImageData, dot: &mut Dot, start_x: i32, start_y: i32) {
    ...
    for y in y0..y_end {
        for x in x0..x_end {
            ...
            let idx = y as usize * img.width as usize + x as usize;
            let weight = (0xff - data[idx]) as u64;
