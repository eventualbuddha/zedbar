...
public:
  ...
  ~ImageScanner() { zbar_image_scanner_destroy(_scanner); }

  /// cast to C image_scanner object
  operator zbar_image_scanner_t *() const { return (_scanner); }
  ...
  int set_config(zbar_symbol_type_t symbology, zbar_config_t config,
                 int value) {
    return (zbar_image_scanner_set_config(_scanner, symbology, config, value));
  }
  ...
  int get_config(zbar_symbol_type_t symbology, zbar_config_t config,
                 int &value) {
    return (zbar_image_scanner_get_config(_scanner, symbology, config, &value));
  }
  ...
  /// see zbar_scan_image()
  int scan(Image &image) { return (zbar_scan_image(_scanner, image)); }
