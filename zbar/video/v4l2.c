...
static inline int v4l2_probe_formats(zbar_video_t *vdo) {
  ...
  for (desc.index = 0; desc.index < V4L2_FORMATS_MAX; desc.index++) {
    ...
    if (desc.flags & V4L2_FMT_FLAG_EMULATED) {
      vdo->emu_formats =
          realloc(vdo->emu_formats, (n_emu_formats + 2) * sizeof(uint32_t));
      vdo->emu_formats[n_emu_formats++] = desc.pixelformat;
    } else {
      vdo->formats = realloc(vdo->formats, (n_formats + 2) * sizeof(uint32_t));
  ...
  zprintf(1, "current format: %.4s(%08x) %u x %u%s (line=0x%x size=0x%x)\n",
          (char *)&pix->pixelformat, pix->pixelformat, pix->width, pix->height,
      ...
      if (frmsize.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
        ...
        for (i = 0; i <= 4; i++) {
          ...
          height =
              ...
              i * (frmsize.stepwise.max_height - frmsize.stepwise.min_height) /
                  4;
...
int _zbar_v4l2_probe(zbar_video_t *vdo) {
  ...
  if (v4l2_ioctl(vdo->fd, VIDIOC_QUERYCAP, &vcap) < 0)
    return (
        err_capture(vdo, SEV_WARNING, ZBAR_ERR_UNSUPPORTED, __func__,
                    "video4linux version 2 not supported (VIDIOC_QUERYCAP)"));

  zprintf(1, "%.32s on %.32s driver %.16s (version %u.%u.%u)\n", vcap.card,
          (vcap.bus_info[0]) ? (char *)vcap.bus_info : "<unknown>", vcap.driver,
          (vcap.version >> 16) & 0xff, (vcap.version >> 8) & 0xff,
          vcap.version & 0xff);
