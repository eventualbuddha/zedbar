...
#define CHECK_COM_ERROR(hr, msg, stmt)                                         \
  if (FAILED(hr)) {                                                            \
    ...
    FormatMessage(                                                             \
        ...
        NULL, hr, 0 /* MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US) */,        \
        (LPSTR) & sysmsg, 0, NULL);                                            \
...
/// Access the BITMAPINFOHEADER in the given AM_MEDIA_TYPE,
/// access is const
static inline const BITMAPINFOHEADER *
dshow_caccess_bih(const AM_MEDIA_TYPE *mt) {
  VIDEOINFOHEADER *vih = (VIDEOINFOHEADER *)mt->pbFormat;
  return &vih->bmiHeader;
}
...
static void dump_formats(zbar_video_t *vdo) {
  ...
  while (*fmt) {
    zprintf(8, "  %.4s / %.4s, resolutions: %lu\n", (char *)&int_fmt->fourcc,
            (char *)fmt, int_fmt->resolutions.cnt);
    fmt++;
    int_fmt++;
  }
}
...
HRESULT __stdcall zbar_samplegrabber_cb_SampleCB(ISampleGrabberCB *_This,
                                                 double sampletime,
                                                 IMediaSample *sample);
// note: original MS version expects a long for the buffer length, wine version
// a LONG
// HRESULT __stdcall zbar_samplegrabber_cb_BufferCB(ISampleGrabberCB* _This,
// double sampletime, BYTE* buffer, long bufferlen);
HRESULT __stdcall zbar_samplegrabber_cb_BufferCB(ISampleGrabberCB *_This,
                                                 double sampletime,
...
HRESULT __stdcall zbar_samplegrabber_cb_BufferCB(ISampleGrabberCB *_This,
  ...
  zprintf(16, "got sample no %ld: %p (%ld), thr=%04lx\n", grabbed_count, buffer,
          bufferlen, _zbar_thread_self());
  ...
  if (img) {
    zprintf(16, "copying into img: %p (srcidx: %d, data: %p, len: %ld)\n", img,
            img->srcidx, img->data, img->datalen);
...
static zbar_image_t *dshow_dq(zbar_video_t *vdo) {
  ...
  zprintf(16, "img: %p (srcidx: %d, data: %p, len: %ld), thr=%04lx\n", img,
          img ? img->srcidx : 0, img ? img->data : NULL, img ? img->datalen : 0,
          _zbar_thread_self());
...
static int dshow_set_format(zbar_video_t *vdo, uint32_t fmt) {
  ...
  hr = IAMStreamConfig_GetStreamCaps(state->camstreamconfig, int_fmt->idx_caps,
                                     &mt, caps);
  free(caps);
  ...
  if (vdo->formats[fmt_ind] != state->int_formats[fmt_ind].fourcc) {
    ...
    vdo->datalen = BMP_SIZE(vdo->width, vdo->height, mjpg_conversion_fmt_bpp);
  }
...
static int dshow_init(zbar_video_t *vdo, uint32_t fmt) {
  ...
  if (state->int_formats[fmt_ind].fourcc == fourcc('M', 'J', 'P', 'G')) {
    ...
    hr = ICaptureGraphBuilder2_RenderStream(
        state->builder, NULL, &MEDIATYPE_Video, (IUnknown *)mjpgdecompressor,
        state->grabberbase, state->nullrenderer);
  ...
  // final media type from sample grabber's input pin;
  {
    ...
    CHECK_COM_ERROR(hr, "couldn't query input media type from sample grabber",
                    goto cleanup1)
  ...
  // hence, keeping a pointer to the original data (iomode VIDEO_MMAP) is a bad
  // idea in a multi-threaded environment. real case szenario on a windows 7
  // tablet with intel atom: with vdo->iomode=VIDEO_MMAP and vdo->num_images=1:
  // 1) directshow graph provides a sample, sets the data pointer of
  // vdo->state->img 2) dshow_dq (called from
  // proc_video_thread()/zbar_video_next_image()) fetches the image and makes a
  // copy of it 3) directshow graph provides the next sample, sets the data
  // pointer of vdo->state->img 4) dshow_nq (called from ) resets the data
  // pointer of vdo->state->img (nullptr) 5) dshow_dq returns vdo->state->img
  // without data (nullptr)
  //
  // now, we could deal with this special case, but zbar_video_next_image()
  // makes a copy of the sample anyway when vdo->num_images==1 (thus VIDEO_MMAP
  // won't save us anything); therefore rather use image buffers provided by
  // zbar (see video_init_images())
  vdo->iomode = VIDEO_USERPTR;
  // keep zbar's default
  // vdo->num_images = ZBAR_VIDEO_IMAGES_MAX;

  // note: vdo->format and vdo->datalen have been set accordingly in
  // dshow_set_format()

  zprintf(3, "initialized video capture: %.4s(%08x), %d frames/s\n",
...
static int dshow_determine_formats(zbar_video_t *vdo) {
  ...
  zprintf(6,
          "number of formats/resolutions supported by the camera as reported "
          "by directshow: %d\n",
          resolutions);
  ...
  // this is actually a VIDEO_STREAM_CONFIG_CAPS structure, which is mostly
  // deprecated anyway, so we just reserve enough buffer but treat it as opaque
  // otherwise
  caps = malloc(state->caps_size);
  for (i = 0; i < resolutions; ++i) {
    ...
    if (dshow_has_vih(mt)) {
      ...
      if (is_supported) {
        ...
        resolution_list_add(&state->int_formats[j].resolutions, &resolution);
      }
    }
...
static IBaseFilter *dshow_search_camera(const char *dev) {
  ...
  int reqid = -1;
  if ((!strncmp(dev, "/dev/video", 10) || !strncmp(dev, "\\dev\\video", 10)) &&
  ...
  hr = ICreateDevEnum_CreateClassEnumerator(
      devenumerator, &CLSID_VideoInputDeviceCategory, &enummoniker, 0);
...
int _zbar_video_open(zbar_video_t *vdo, const char *dev) {
  ...
  hr = CoCreateInstance(&CLSID_CaptureGraphBuilder2, NULL, CLSCTX_INPROC_SERVER,
                        &IID_ICaptureGraphBuilder2, (void **)&state->builder);
  ...
  // TODO:
  // finding the streamconfig interface on the camera's preview output pin
  // (specifying PIN_CATEGORY_PREVIEW, MEDIATYPE_Video) should work according to
  // the documentation but it doesn't. Because devices may have separate pins
  // for capture and preview or have a video port pin (PIN_CATEGORY_VIDEOPORT)
  // instead of a preview pin, I do hope that we get the streamconfig interface
  // for the correct pin
